...
export default function HeroSection() {
  ...
  return (
    <section className="relative overflow-hidden gradient-hero">
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-r from-[#0b1220]/90 via-[#0b1220]/75 to-[#0b1220]/40" />
      ...
      <div className="container relative z-10 pt-14 pb-20 md:pt-24 md:pb-28">
        <div className="max-w-2xl">
          ...
          >
            <div className="inline-flex items-center gap-1.5 bg-primary/20 text-white px-3 py-1 rounded-full text-xs mb-4 backdrop-blur-sm border border-white/10">
              <span className="w-1.5 h-1.5 rounded-full bg-primary" />
              <span>Nigeria&apos;s Online Marketplace</span>
            </div>
            <h1 className="font-heading text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-[1.05] tracking-tight">
              Buy, Sell &amp;{' '}
              <span className="text-primary">Grow</span>
              <br className="hidden sm:block" /> With Confidence
            </h1>
            <p className="mt-4 text-white/70 text-sm md:text-lg max-w-md">
            ...
            <div className="flex flex-wrap gap-3 mt-7">
              ...
              <Link to="/category/deals-refurbished">
                <Button size="lg" variant="outline" className="font-heading font-semibold border-white/30 text-white hover:bg-white/10 hover:text-white">
                  View Deals
                </Button>
              </Link>
            </div>
      ...
      <div className="relative z-10 border-t border-white/10">
        <div className="container grid grid-cols-3 divide-x divide-white/15 py-4 md:py-5">
...
function Stat({ icon, value, label }: { icon: React.ReactNode; value: string; label: string }) {
  return (
    <div className="flex flex-col items-center md:flex-row md:items-center gap-1 md:gap-2 px-2 text-center md:text-left">
      ...
      <span className="font-heading font-bold text-sm sm:text-base text-white leading-none">{value}</span>
      <span className="text-[10px] sm:text-xs text-white/60 leading-none">{label}</span>
    </div>
  );
}
